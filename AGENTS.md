...
- Treat modules under `zk/core/` as the stable, policy-free semantic and external-interaction API. Do not add application-specific formats, vocabularies, extraction rules, diagnostic policies, effect handlers, concrete transports, or editor behavior to these modules.
- `zk/core/eval.typ` defines the shared evaluation-boundary contract: `announce(tag, value)` declares externally observable values, and `inspect(content)` marks source provenance for externalization within announcements. These inert metadata declarations belong to core; keep tags open and preserve the `eval.announcement` / `eval.inspect` protocol. The Rust `zk-eval` host implements collection and source-span externalization outside core, without interpreting application semantics.
...
- Keep application-specific diagnostics and effect protocols, effect execution, concrete transports, editor integration, and preview/debug output in separate modules that consume the core and selected rule implementations. Core can be distributed as a self-contained Typst library; external interaction additionally requires a host implementing its evaluation-boundary protocol.
